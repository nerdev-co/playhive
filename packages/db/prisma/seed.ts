...
async function main() {
  ...
  let guestUser = await db.orm.public.User.where({ username: 'guest_001' }).first();
  if (!guestUser) {
  ...
  let room = await db.orm.public.GameRoom.where({ name: 'Ludo Match #1' }).first();
  if (!room) {
  ...
  const existingEvents = await db.orm.public.GameEvent.where({ matchId: match.id }).all();
  if (existingEvents.length === 0) {
    const events = [
      { version: 1n, seat: 0, event: { type: 'dice', value: 4 }, playerId: user1.id },
      { version: 2n, seat: 0, event: { type: 'move', token: 0, from: 0, to: 4 }, playerId: user1.id },
      { version: 3n, seat: 1, event: { type: 'dice', value: 6 }, playerId: user2.id },
      { version: 4n, seat: 1, event: { type: 'move', token: 0, from: 0, to: 6 }, playerId: user2.id },
      { version: 5n, seat: 1, event: { type: 'dice', value: 3 }, playerId: user2.id },
      { version: 6n, seat: 1, event: { type: 'move', token: 0, from: 6, to: 9 }, playerId: user2.id },
    ];

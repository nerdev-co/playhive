...
**`apps/`**
- `apps/server` — game server (Go or Node)
...
| Area | What to build |
|------|--------------|
| **Transport** | WebSocket server with the envelope format from `protocol.md` |
| **Auth** | Guest token flow (`AUTH` → `AUTH_OK`); stateless JWT or session token |
...
| **Game engine interface** | Common interface both chess and ludo engines implement (`applyAction`, `getState`, `gameOver`) |
...
| **Redis** | Presence TTL, matchmaking queues, dedup zsets |
| **Event flushing** | Async batch writer (1s or 50 events) |
| **Archive job** | Move finished matches from memory to Postgres, evict room state |
...
| Area | What to build |
|------|--------------|
| **WebSocket client** | Envelope send/receive, reconnect/resume logic |
| **Lobby** | Room list, create/join, matchmaking queue UI |
| **Game UI** | Board rendering, move input, legal move highlights |
| **WebRTC** | `MEDIA_OFFER`/`MEDIA_ANSWER`/`MEDIA_ICE` relay for voice/video |
...
3. **`apps/server`** — minimal WebSocket server that can create a room and apply one Ludo action.
